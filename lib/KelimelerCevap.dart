import 'package:sozluk_app/Kelimeler.dart';

class KelimelerCevap{
  int success;
  List<Kelimeler> kelimelerListesi;

  KelimelerCevap(this.success, this.kelimelerListesi);

  factory KelimelerCevap.fromJson(Map<String,dynamic> json){
    var jsonArray = json["kelimeler"] as List;
    List<Kelimeler> kelimeListesi = jsonArray.map((jsonArrayNesnesi) => Kelimeler.fromJson(jsonArrayNesnesi)).toList();
    return KelimelerCevap(json["success"] as int,kelimeListesi);
  }
}